...
3. Wywoływana jest metoda TrafficLightControler.nextStep(), która podejmuje decyzję o wyborze optymalnej fazy świateł:
    - zostają uaktualnione współczynniki wag za pomocą których wybierana jest faza w zależności od natężenia aut, czasu oczekiwania pojazdów, czasu oczekiwania pieszych
    - obliczane jest względne natężnie ruchu na poszczególnych kierunkach - procentowy udział liczby aut w każdym kierunku względem wszystkich i na jego podstawie sprawdzany jest czy występuje nierównowaga w natężeniu - do sprawdzenia tego wykorzystywane są *metody statystyczne* - obliczana jest średnia i odchylenie standardowe 
        ...
        - na końcu rozważani są piesi którzy mają własny współcznynnik premiowania faza w których mogą się przemieszczać się - brany jest czas oczekiwania, który przy wydłużaniu się zostaje zwiększony.
    - po optymalizacji współczynników rozpoczyna się proces decyzyjny jaką fazę wybrać za pomocą metody selectBestPhase() - która ocenia każdą fazę w kontekście obecnej sytuacji na skrzyżowaniu - evaluatePhase() - wyliczając *score* jako sumę iloczynu wcześniej obliczanych współczynników i liczby pojazdów, ich czasów oczekiwania i taki sam iloczyn dla pieszych przemnożony przez współczynnik równoważacy ich
    - na podstawie obliczonych *score'ów* wybierana jest najlepsza faza
    - algorytm implementuje także mechanizm uczenia się z przeszłości, śledzący efektywność każdej fazy
...
- γ (gamma) - waga dla priorytetyzacji pieszych

W przypadku nierównomiernego rozkładu ruchu (sprawdzanego za pomocą odchylenia standardowego > 0.2):
- α zwiększana jest do 1.5, aby priorytetyzować obsługę zatłoczonych kierunków
...
- β zwiększana jest do 1.5, aby priorytetyzować kierunki z długim czasem oczekiwania
...
2. Wykrywa nierównowagi w ruchu używając metod statystycznych (odchylenie standardowe)
3. Zapobiega zbyt długim czasom oczekiwania przez priorytetyzację kierunków z długo czekającymi pojazdami
...
Algorytm jest efektywny w różnych warunkach ruchu i adaptuje się zarówno do równomiernego natężenia, jak i sytuacji z nierównowagami w przepływie pojazdów.
...
- Symulację różnych typów rzeczywistych skrzyżowań (3-wlotowe, 4-wlotowe, z różnymi układami pasów)
...
System implementuje kompleksowe rozwiązanie dla ruchu pieszego, które priorytetyzuje bezpieczeństwo:
...
- Zastosowano dynamiczną priorytetyzację przejść dla pieszych - im dłużej piesi czekają, tym wyższy priorytet otrzymują (współczynnik γ)
...
System rozróżnia przejścia dla pieszych na różnych wlotach skrzyżowania i optymalizuje ich obsługę niezależnie, co pozwala na bardziej granularne sterowanie ruchem pieszym.
...
- Mechanizm działa w czasie rzeczywistym, oceniając sytuację na skrzyżowaniu w każdym kroku symulacji
...
- Zmniejszenie emisji spalin poprzez ograniczenie czasu pracy silników na biegu jałowym
